...
Функция ПрочитатьСообщения(КоличествоСообщенийКЧтению) Экспорт
	...
	Пока МассивСообщений.Количество() < КоличествоСообщенийКЧтению Цикл
		...
		Мета = ЗначениеИзJSON(МетаданныеJSON);
		
		НовоеСообщение = НовоеПрочитанноеСообщение(Данные, Мета.key, Мета.headers, Мета.partition, Мета.offset);
...
Функция ПолучитьНастройку(ИдентификаторНастройки)
	...
	Настройка = Новый Структура("Идентификатор, Отправка, Получение, Брокер, Топик, ГруппаКонсюмера,
		|ТаймаутОпераций, АвтоКоммит, УровеньЛогирования, КоличествоХранимыхЛогФайлов, 
