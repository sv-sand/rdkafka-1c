...
Функция НачатьЛогирование(УровеньЛогирования) Экспорт
	...
	Результат = Компонента.НачатьЛогирование(ЛогФайл, УровеньЛогирования);
	Если НЕ Результат Тогда
...
//
Процедура ОстановитьЛогирование() Экспорт
	
	Результат = Компонента.ОстановитьЛогирование();
	Если НЕ Результат Тогда
		ТекстСообщения = СтрШаблон("Не удалось начать логирование: %1", Компонента.ОписаниеОшибки);
		ВызватьИсключение ТекстСообщения;
	КонецЕсли;
...
Процедура НастроитьКонфигурацию()
	...
	Компонента.УстановитьПараметр("auto.offset.reset", "smallest");	
	Если Настройка.АвтоКоммит Тогда
		Компонента.УстановитьПараметр("enable.auto.commit", "true");	
	Иначе	
		Компонента.УстановитьПараметр("enable.auto.commit", "false");
	КонецЕсли;
	
	Компонента.УстановитьПараметр("security.protocol", Настройка.ПротоколБезопасности);
	...
	Если Настройка.ПротоколБезопасности = "sasl_plaintext" ИЛИ Настройка.ПротоколБезопасности = "sasl_ssl" Тогда
		Компонента.УстановитьПараметр("sasl.mechanisms", Настройка.МеханизмSASL);
		Компонента.УстановитьПараметр("sasl.username", Настройка.ИмяПользователяSASL);
		Компонента.УстановитьПараметр("sasl.password", Настройка.ПарольSASL);
	КонецЕсли;
